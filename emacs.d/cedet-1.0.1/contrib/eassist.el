...
(defun get-buffer-regexp (b)
  (dolist (bf (buffer-list))
    (if (string-match b (buffer-name bf))
        (return bf))))
(defun get-buffers-regexp (b)
  (let (res)
    (dolist (bf (buffer-list) res)
      (if (string-match b (buffer-name bf))
          (add-to-list 'res bf)))))
(defun switch-into-buffers-completing (buffers)
  (let* ((names (mapcar 'buffer-name buffers))
    ...
    (if single
      ...
      (progn
        (setq choice (completing-read "Multiply Target, Which One? :" names
                                      nil t (try-completion "" names)))
        (switch-to-buffer (get-buffer choice))))))
(defun switch-into-files-completing (dirs fn)
  (let* ((single (= (length dirs) 1))
         choice)
    (if single
        (find-file (concat (car dirs) fn))
      (progn
        (setq choice (completing-read (format
                                       "Multiply Target for file %s, Which One? :"
                                       fn
                                       )
                                      dirs
                                      nil t (try-completion "" dirs)))
        (find-file (concat choice fn) )))))
(defun find-file-around-buffers-and-file (fn)
  (remove-if-not '(lambda (dir) (file-exists-p (concat dir fn)))
                 (remove-duplicates
                  (mapcar 'file-name-directory
                          (remove-if 'null (mapcar '(lambda (b) (or
                                                                 (buffer-file-name b)
...
for example *.hpp <--> *.cpp."
  ...
  (let* ((ext (file-name-extension (buffer-file-name)))
         ...
         (base-path (file-name-sans-extension (buffer-file-name)))
         (count-ext (cdr (find-if (lambda (i) (string= (car i) ext)) eassist-header-switches))))
    (cond
     (count-ext
      (unless
          (or
           (loop for b in (mapcar (lambda (i) (concat base-name "." i)) count-ext)
                 when (get-buffers-regexp (concat "^" (regexp-quote b)))
                 return (switch-into-buffers-completing
                         (get-buffers-regexp (concat "^" (regexp-quote b)))))
           (loop for c in (mapcar (lambda (count-ext) (concat base-path "." count-ext)) count-ext)
                 when (file-exists-p c) return (find-file c))
           (loop with final-dirs = nil
                 for c in (mapcar (lambda (count-ext) (concat base-name "." count-ext)) count-ext)
                 when (setq final-dirs (find-file-around-buffers-and-file c))
                 return (switch-into-files-completing final-dirs c))
           )
        (message "There is no corresponding pair (header or body) file.")))
     (t
      (message "It is not a header or body file! See eassist-header-switches variable.")))))
